...
The CI pipeline runs automatically on every push and PR. No setup needed for unit tests — they run with zero secrets. E2E tests trigger only on PRs to `main` and need five secrets configured once.
...
| `E2E_TEST_EMAIL` | Email of your dedicated test account |
| `E2E_TEST_PASSWORD` | Password of your dedicated test account |
| `GROQ_API_KEY` | Optional — AI routes are mocked in E2E tests |

### CI jobs

| Job | Trigger | What it does |
|-----|---------|-------------|
| Unit tests | Every push | Runs `npm test` — 101 pure-function tests, no secrets needed |
| TypeScript | Every push | `tsc --noEmit` — catches type errors without a full build |
| E2E tests | PRs to `main` only | Builds the app, starts the server, runs all Playwright specs |

### What E2E tests validate end-to-end

- Login redirects to `/today`, invalid credentials stay on login page
- Today page loads action card, all 4 outcome chips render, submitting shows score delta
- Billing: Paystack verify returns correct plan, Paddle verify rejects bad transactions
- Explore: seed fallback works, live data shows `● live`, stage filters work

---

## Geo metadata (flag + city)

User geo data is written automatically in two places:

1. **On onboarding completion** — `app/onboarding/page.tsx` calls `POST /api/user/geo` after the project is created.
2. **On login** — `app/auth/login/page.tsx` calls `POST /api/user/geo` for any existing user whose `flag` metadata is not yet set.

The geo route reads Vercel's built-in IP geo headers (`x-vercel-ip-country`, `x-vercel-ip-city`) — **no third-party API key or billing required**. These headers are populated automatically on all Vercel deployments.

In local development the headers are absent → the route returns `{ ok: true, skipped: "no admin env" }` and nothing is written. This is correct behaviour — no fake data pollutes development.

The `feed_events` table receives real `flag` and `city` values from the first reflect submission after a user logs in on Vercel. Users who sign up and reflect within the same session will have their geo set immediately.
