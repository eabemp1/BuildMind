...
export default function NotificationsPage() {
  ...
  return (
    <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold bm-text">Notifications</h2>
      ...
      <Card className="glass-panel panel-glow">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="bm-text">Inbox</CardTitle>
          <Button
            variant="outline"
        ...
        <CardContent className="space-y-2">
          {isLoading ? <p className="text-sm bm-text2">Loading notifications...</p> : null}
          {error ? <p className="text-sm text-rose-400">{(error as Error).message}</p> : null}

          {items.map((item) => (
            <div key={item.id} className="flex items-center justify-between rounded-lg border border-[var(--bm-border2)] bg-white/5 px-3 py-2">
              <div>
                <p className="text-sm font-medium bm-text">{item.message}</p>
                <p className="text-xs bm-text3">{new Date(item.created_at).toLocaleString()}</p>
              </div>
              {!item.is_read ? (
                <Button
                  variant="outline"
                ...
                <span className="text-xs bm-text3">Read</span>
              )}
            </div>
          ))}

          {!isLoading && !items.length ? (
            <div className="rounded-lg border border-[var(--bm-border2)] bg-white/5 p-4">
              <p className="text-sm font-medium bm-text">No notifications yet.</p>
              <p className="text-sm bm-text2">You will see milestone, task, and AI updates here.</p>
            </div>
          ) : null}
        </CardContent>
      </Card>
