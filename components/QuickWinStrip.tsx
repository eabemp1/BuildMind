...
export function QuickWinStrip({ stage = "Idea", streak = 0, completedToday }: Props) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18, duration: 0.3 }}
      style={{ display: "flex", flexDirection: "column", gap: 8, padding: "12px 16px", background: "var(--bm-bg2)", border: "1px solid var(--bm-border)", borderRadius: 10, marginBottom: 14 }}>
      {liveCount != null && liveCount > 1 && (
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
          <span style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--bm-accent)", display: "inline-block", flexShrink: 0 }} />
          <span style={{ fontSize: 12, color: "var(--bm-text3)" }}>
            <strong style={{ color: "var(--bm-text2)", fontWeight: 600 }}>{liveCount.toLocaleString()} founders</strong>{" "}completed their task today.
          </span>
        </div>
      )}
      <div style={{ display: "flex", alignItems: "flex-start", gap: 7 }}>
        <span style={{ fontSize: 12, color: "var(--bm-text4)", flexShrink: 0, paddingTop: 1 }}>📊</span>
        <span style={{ fontSize: 12, color: "var(--bm-text3)", lineHeight: 1.5 }}>{benchmark}</span>
      </div>
      {streakCopy && (
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
