 ...
 *
 * The "ghost" weekly goal — a persistent tracker that follows the founder
 * through the Today page. Shows:
 *  - Their stated (or AI-calibrated) goal for the week
 *  - How they're tracking vs their score target
 *  - Tasks done vs tasks target
 *  - Whether they're surpassing, on track, or behind
 *
 * The ghost is subtle — it haunts, not shouts.
 */

import { useState, useEffect, useCallback } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { Target, Sparkles, ChevronDown, ChevronUp, X, Check, TrendingUp, AlertTriangle } from "lucide-react";
...
export default function GhostGoalBanner({
  ...
  const fetchGoal = useCallback(async () => {
    ...
    try {
      ...
      if (json.ok && json.data) {
        // Sync current score into the goal display
  ...
  // Sync score update when currentScore changes
  useEffect(() => {
    if (!goal || !projectId) return;
    // Update on the server (best-effort)
      ...
      >
        <AnimatePresence mode="wait">
          {!settingGoal ? (
            ...
            >
              <Target size={13} color="var(--bm-text4)" style={{ flexShrink: 0 }} />
              <span style={{ fontSize: 12, color: "var(--bm-text4)", flex: 1 }}>
                Set a goal for this week — it&apos;ll follow you like a ghost through every check-in.
              </span>
              <button
                onClick={() => setSettingGoal(true)}
                style={{
                  fontSize: 11, fontWeight: 700, color: "var(--bm-accent)",
            ...
            >
              ...
              />
              {error && (
                <p style={{ fontSize: 11, color: "var(--bm-red, #E05555)", margin: 0 }}>{error}</p>
              )}
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                <button
                  ...
                  style={{
                    ...
                    background: "var(--bm-accent)", color: "var(--bm-bg)",
                    fontSize: 12, fontWeight: 700, cursor: saving || !customText.trim() ? "not-allowed" : "pointer",
                    opacity: saving || !customText.trim() ? 0.5 : 1,
                    fontFamily: "inherit",
                  }}
                >
                ...
                <button
                  ...
                  style={{
                    ...
                    background: "var(--bm-bg3)", color: "var(--bm-text2)",
                    fontSize: 12, fontWeight: 600, cursor: calibrating ? "not-allowed" : "pointer",
  ...
  // ── Goal exists — show the ghost tracker ────────────────────────────────────
  const scorePct  = Math.min(100, Math.round((goal.current_score / Math.max(1, goal.target_score)) * 100));
  const taskPct   = Math.min(100, Math.round((goal.tasks_done / Math.max(1, goal.target_tasks)) * 100));
  const surpassed = goal.status === "surpassed";
  const behind    = goal.current_score < goal.target_score * 0.5 && goal.tasks_done < Math.floor(goal.target_tasks * 0.5);
  ...
  return (
    <motion.div
      ...
      style={{
        marginBottom: 14,
        borderRadius: 12,
        border: `1px solid ${surpassed ? "rgba(74,184,176,0.3)" : behind ? "rgba(224,85,85,0.2)" : "var(--bm-border)"}`,
        background: surpassed
          ? "rgba(74,184,176,0.06)"
          : behind
          ? "rgba(224,85,85,0.04)"
          : "var(--bm-bg2)",
        overflow: "hidden",
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            fontSize: 10, fontWeight: 700, color: ghostColor,
            textTransform: "uppercase", letterSpacing: "0.09em", marginBottom: 1,
          }}>
          ...
          <div style={{
            fontSize: 12, color: "var(--bm-text2)",
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
          ...
          {expanded ? <ChevronUp size={13} color="var(--bm-text4)" /> : <ChevronDown size={13} color="var(--bm-text4)" />}
        </div>
      </button>
            ...
            <div style={{
              ...
              <div style={{ marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                  <span style={{ fontSize: 10, color: "var(--bm-text3)", textTransform: "uppercase", letterSpacing: "0.07em" }}>
                    Score progress
                  </span>
                  <span style={{ fontSize: 10, color: ghostColor, fontWeight: 700 }}>
              ...
              <div style={{ marginBottom: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                  ...
                  <span style={{ fontSize: 10, color: ghostColor, fontWeight: 700 }}>
                    {goal.tasks_done} / {goal.target_tasks}
                    {taskPct >= 100 && " ✓"}
                  </span>
                </div>
                <div style={{ height: 5, borderRadius: 99, background: "var(--bm-bg3)", overflow: "hidden" }}>
              ...
              <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
                <button
                  onClick={() => { setGoal(null); setSettingGoal(true); setExpanded(false); }}
                  style={{
                    fontSize: 10, color: "var(--bm-text4)", background: "none", border: "none",
                    cursor: "pointer", padding: "3px 0", fontFamily: "inherit",
                  }}
                >
                  Change goal
                </button>
