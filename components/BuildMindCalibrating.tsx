/**
 * Renders in place of under-signal surfaces (Insights, Break My Startup,
 * Patterns tab) when reflections.count < 7. Converts "nothing here yet"
...
export function BuildMindCalibrating({ count, surface = "insights" }: Props) {
  ...
  return (
    <motion.div
      ...
      style={{
        maxWidth: 480, margin: "48px auto", padding: "28px 28px 24px",
    ...
    >
      ...
      <p style={{ fontSize: 15, fontWeight: 600, color: "var(--bm-text)", letterSpacing: "-0.02em", margin: "0 0 6px" }}>
        BuildMind is learning how you build.
      </p>
      <p style={{ fontSize: 13, color: "var(--bm-text3)", lineHeight: 1.55, margin: "0 0 20px" }}>
          ...
          {Array.from({ length: total }).map((_, i) => (
            <motion.div key={i}
              ...
              style={{
                ...
                background: i < filled ? "var(--bm-accent)" : "var(--bm-bg4)",
                border: `1px solid ${i < filled ? "var(--bm-accent-bd)" : "var(--bm-border)"}`,
                transition: "background 0.3s",
              }}
            />
          ))}
        </div>
        <p style={{ fontSize: 11, color: "var(--bm-text3)", fontFamily: "'JetBrains Mono', monospace", margin: 0 }}>
      ...
      <div style={{
        ...
        borderRadius: 10, padding: "14px 16px", textAlign: "left", marginBottom: 18,
      }}>
        <p style={{ fontSize: 10, fontWeight: 700, color: "var(--bm-text4)", textTransform: "uppercase", letterSpacing: "0.08em", margin: "0 0 10px" }}>
          After {total} reflections, you unlock:
        </p>
        <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
          {unlockCopy.map((item, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 11, color: i < filled ? "var(--bm-accent)" : "var(--bm-text4)", flexShrink: 0, fontWeight: 700 }}>
                {i < filled ? "✓" : "○"}
              </span>
              <span style={{ fontSize: 13, color: i < filled ? "var(--bm-text2)" : "var(--bm-text3)" }}>{item}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {filled < total && (
        <p style={{ fontSize: 12, color: "var(--bm-text3)", lineHeight: 1.5, margin: 0 }}>
          Tonight&apos;s reflection is{" "}
          <span style={{ color: "var(--bm-accent)", fontWeight: 600 }}>signal {nextSignal}</span>.
        </p>
      )}
    </motion.div>
  );
