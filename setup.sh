...
echo ""
echo "ADMIN SETUP (first deploy only):"
echo "  To access the admin panel, set is_admin=true for your account."
echo "  Option A — Supabase SQL editor:"
echo "    ALTER DATABASE postgres SET app.admin_email = 'you@example.com';"
echo "    -- then run: supabase/migrations/20260510100000_admin_bootstrap.sql"
echo ""
echo "  Option B — direct SQL:"
echo "    UPDATE public.profiles SET is_admin = true"
echo "    WHERE id = (SELECT id FROM auth.users WHERE email = 'you@example.com');"
echo ""
npm run dev
