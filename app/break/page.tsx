    ...
    <div style={{ position:"relative", width:size, height:size, margin:"0 auto" }}>
      ...
      <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
        ...
        <motion.span initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:1.1 }}
          style={{ fontSize:10, color:"rgba(255,255,255,0.35)", textTransform:"uppercase", letterSpacing:"0.12em", marginTop:4 }}>{label}</motion.span>
      </div>
    </div>
  );
}
    ...
    <div style={{ textAlign:"center", padding:"60px 20px" }}>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={stepIdx} initial={{ opacity:0, y:6 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-6 }} transition={{ duration:0.3 }}
          style={{ fontSize:14, color:"rgba(255,255,255,0.5)", fontFamily:"monospace" }}>{LOADING_STEPS[stepIdx]}</motion.div>
      </AnimatePresence>
      <div style={{ display:"flex", justifyContent:"center", gap:6, marginTop:24 }}>
        {LOADING_STEPS.map((_,i) => <div key={i} style={{ width:6, height:6, borderRadius:"50%", background:i<=stepIdx?"#ef4444":"rgba(255,255,255,0.1)", transition:"background 0.3s" }} />)}
      </div>
    </div>
  );
}
    ...
    <div style={{ minHeight:"100vh", background:"#080810", color:"#f0f0f5", fontFamily:"system-ui,-apple-system,sans-serif" }}>
      ...
      <div style={{ maxWidth:680, margin:"0 auto", padding:"20px 20px 0", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        ...
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          <Link href="/auth/login" style={{ fontSize:12, color:"rgba(255,255,255,0.4)", textDecoration:"none", padding:"6px 12px" }}>Sign in</Link>
            ...
            <motion.div key="input" initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-12 }}>
              ...
              <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:16, overflow:"hidden" }}>
                <div style={{ padding:"20px 24px", borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
                  <label style={{ fontSize:11, color:"rgba(255,255,255,0.3)", fontFamily:"monospace", letterSpacing:"0.1em", textTransform:"uppercase", display:"block", marginBottom:10 }}>What are you building? *</label>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
                  <div style={{ padding:"16px 24px", borderRight:"1px solid rgba(255,255,255,0.06)" }}>
                    <label style={{ fontSize:11, color:"rgba(255,255,255,0.3)", fontFamily:"monospace", letterSpacing:"0.1em", textTransform:"uppercase", display:"block", marginBottom:8 }}>Who is it for? (optional)</label>
                  ...
                  <div style={{ padding:"16px 24px" }}>
                    <label style={{ fontSize:11, color:"rgba(255,255,255,0.3)", fontFamily:"monospace", letterSpacing:"0.1em", textTransform:"uppercase", display:"block", marginBottom:8 }}>Core problem? (optional)</label>
                ...
                <div style={{ padding:"16px 24px" }}>
                  ...
                  <button onClick={handleRun} disabled={!canRun} style={{
                    ...
                    background:canRun?"linear-gradient(135deg,#7f1d1d,#ef4444)":"rgba(255,255,255,0.05)",
                    border:"none", borderRadius:10, color:canRun?"#fff":"rgba(255,255,255,0.2)",
                  ...
                  <p style={{ textAlign:"center", fontSize:11, color:"rgba(255,255,255,0.2)", marginTop:10, marginBottom:0 }}>Free · No account needed · Takes ~30 seconds</p>
                </div>
              </div>
            </motion.div>
          )}
          ...
          {phase==="result" && result && (
            <motion.div key="result" initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }}>
              ...
              <div style={{ textAlign:"center", marginBottom:36 }}>
                <div style={{ fontSize:11, color:"rgba(255,255,255,0.3)", fontFamily:"monospace", letterSpacing:"0.12em", textTransform:"uppercase", marginBottom:20 }}>survival probability</div>
                <SurvivalArc score={result.survival_probability} />
                <motion.p initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:1.2 }}
                  style={{ fontSize:14, color:"rgba(255,255,255,0.5)", maxWidth:480, margin:"20px auto 0", lineHeight:1.65 }}>{result.verdict}</motion.p>
              </div>
              ...
              <div style={{ marginBottom:24 }}>
                ...
                {result.kill_reasons.map((r,i)=>(
                  <motion.div key={i} initial={{ opacity:0, x:-12 }} animate={{ opacity:1, x:0 }} transition={{ delay:0.1*i+0.3 }}
                    ...
                    <span style={{ fontSize:13, color:"rgba(255,255,255,0.7)", lineHeight:1.55 }}>{r}</span>
                  </motion.div>
                ))}
              </div>
              ...
              <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.6 }}
                style={{ background:"rgba(129,140,248,0.06)", border:"1px solid rgba(129,140,248,0.2)", borderRadius:12, padding:"16px 18px", marginBottom:24 }}>
                <div style={{ fontSize:11, color:"#818cf8", fontFamily:"monospace", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:8, fontWeight:600 }}>The one thing to do now</div>
                <p style={{ fontSize:14, color:"rgba(255,255,255,0.8)", lineHeight:1.65, margin:0 }}>{result.brutal_advice}</p>
              </motion.div>
                ...
                <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.75 }}
                  ...
                  {result.differentiation_plan.map((item,i)=>(
                    <div key={i} style={{ display:"flex", gap:10, marginBottom:i<result.differentiation_plan.length-1?8:0 }}>
                      ...
                      <span style={{ fontSize:13, color:"rgba(255,255,255,0.6)", lineHeight:1.55 }}>{item}</span>
                    </div>
                  ))}
                </motion.div>
              )}
              ...
              <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.9 }}
                ...
                <button onClick={handleCopy} style={{ padding:"11px 16px", borderRadius:8, background:"transparent", border:"1px solid rgba(255,255,255,0.1)", color:"rgba(255,255,255,0.4)", fontSize:12, cursor:"pointer", fontFamily:"inherit" }}>{copied?"Copied!":"Copy"}</button>
                <button onClick={handleReset} style={{ padding:"11px 16px", borderRadius:8, background:"transparent", border:"1px solid rgba(255,255,255,0.1)", color:"rgba(255,255,255,0.4)", fontSize:12, cursor:"pointer", fontFamily:"inherit" }}>Try another ↺</button>
              </motion.div>
              ...
              <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay:1.1 }}
                ...
                <div style={{ display:"flex", gap:10, justifyContent:"center", flexWrap:"wrap" }}>
                  <Link href={`/auth/login?idea=${encodeURIComponent(idea.slice(0,100))}`}
                    style={{ padding:"12px 28px", borderRadius:9, background:"linear-gradient(135deg,#6366f1,#8b5cf6)", color:"#fff", fontSize:13, fontWeight:700, textDecoration:"none", display:"inline-block" }}>
                    Start building smarter — free →
                  </Link>
                  <Link href="/auth/login" style={{ padding:"12px 20px", borderRadius:9, background:"transparent", border:"1px solid rgba(255,255,255,0.1)", color:"rgba(255,255,255,0.4)", fontSize:13, textDecoration:"none", display:"inline-block" }}>Sign in</Link>
                </div>
                <p style={{ fontSize:11, color:"rgba(255,255,255,0.2)", marginTop:12, marginBottom:0 }}>Free plan · No credit card · 2 minutes to set up</p>
              </motion.div>
            </motion.div>
          )}
        </AnimatePresence>
