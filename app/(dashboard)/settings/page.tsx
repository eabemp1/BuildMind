...
function SettingsInput({ value, onChange, placeholder, type = "text", disabled }: any) {
  return (
    <input value={value} onChange={onChange} placeholder={placeholder} type={type} disabled={disabled}
      style={{ width: "100%", background: "var(--bm-bg3)", border: "1px solid var(--bm-border2)", borderRadius: 10, padding: "10px 14px", fontSize: 13, color: "var(--bm-text)", outline: "none", fontFamily: "inherit", boxSizing: "border-box", transition: "border-color 0.15s", opacity: disabled ? 0.5 : 1 }}
...
function SaveButton({ loading, onClick }: { loading: boolean; onClick: () => void }) {
  return (
    <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.97 }} onClick={onClick} disabled={loading}
      style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 20px", borderRadius: 10, border: "none", background: "var(--grad-primary)", color: "#fff", fontWeight: 600, fontSize: 13, cursor: loading ? "not-allowed" : "pointer", fontFamily: "inherit", opacity: loading ? 0.7 : 1, transition: "opacity 0.15s" }}>
...
function BillingTab() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
          ...
          {isPaid ? (
            ...
            <a href="/upgrade" style={{ display: "flex", alignItems: "center", gap: 7, padding: "10px 18px", borderRadius: 11, border: "none", background: "var(--grad-primary)", color: "#fff", fontWeight: 700, fontSize: 13, textDecoration: "none" }}>
      ...
      {isPaid && (
        <div style={{ background: "var(--bm-bg2)", border: "1px solid var(--bm-border)", borderRadius: 16, padding: "20px 22px" }}>
  ...
  return (
    <div style={{ maxWidth: 740, margin: "0 auto", padding: "28px 24px" }}>
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--bm-text)", letterSpacing: "-0.03em", margin: "0 0 4px" }}>Settings</h1>
        <p style={{ fontSize: 12, color: "var(--bm-text3)", margin: 0 }}>Manage your account, notifications, and AI preferences.</p>
      </motion.div>
              ...
              return (
                <button key={t.id} onClick={() => setTab(t.id)}
                  style={{
                    ...
                    cursor: "pointer", fontFamily: "inherit", textAlign: "left", width: "100%", transition: "all 0.13s",
                  }}
                  ...
                  <Icon size={13} strokeWidth={active ? 2.2 : 1.6} style={{ flexShrink: 0 }} />
                  {t.label}
                </button>
              );
            })}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <AnimatePresence mode="wait">
            <motion.div key={tab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.15 }}>
              ...
              {tab === "profile" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <div style={{ background: "var(--bm-bg2)", border: "1px solid var(--bm-border)", borderRadius: 16, padding: "22px 24px" }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "var(--bm-text)", marginBottom: 20 }}>Public Profile</div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                      ...
                      <div>
                        ...
                        <textarea value={bio} onChange={e => setBio(e.target.value)} rows={3} placeholder="Building in public. Founder of [startup]."
                          style={{ width: "100%", background: "var(--bm-bg3)", border: "1px solid var(--bm-border2)", borderRadius: 10, padding: "10px 14px", fontSize: 13, color: "var(--bm-text)", outline: "none", fontFamily: "inherit", resize: "none", boxSizing: "border-box", lineHeight: 1.55, transition: "border-color 0.15s" }}
                          onFocus={e => { e.target.style.borderColor = "var(--bm-accent-bd)"; }}
                          onBlur={e => { e.target.style.borderColor = "var(--bm-border2)"; }} />
                      </div>
                    </div>
              ...
              {tab === "account" && (
                <div style={{ background: "var(--bm-bg2)", border: "1px solid var(--bm-border)", borderRadius: 16, padding: "22px 24px" }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "var(--bm-text)", marginBottom: 20 }}>Account</div>
              ...
              {tab === "notifications" && FEATURES.notifications && (
                <div style={{ background: "var(--bm-bg2)", border: "1px solid var(--bm-border)", borderRadius: 16, padding: "22px 24px" }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "var(--bm-text)", marginBottom: 20 }}>Notifications</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
                    {[
                      ...
                      <div key={key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 0", borderBottom: "1px solid var(--bm-border)" }}>
                        <div>
                          <div style={{ fontSize: 13, fontWeight: 500, color: "var(--bm-text2)", marginBottom: 2 }}>{label}</div>
                          <div style={{ fontSize: 11, color: "var(--bm-text3)" }}>{desc}</div>
                        </div>
              ...
              {tab === "ai" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <div style={{ background: "var(--bm-bg2)", border: "1px solid var(--bm-border)", borderRadius: 16, padding: "22px 24px" }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "var(--bm-text)", marginBottom: 6 }}>AI Coach Personality</div>
                    <div style={{ fontSize: 12, color: "var(--bm-text3)", marginBottom: 18 }}>Choose how your AI Coach communicates with you.</div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                      {[
                        ...
                        <button key={opt.id} onClick={() => setAiPersonality(opt.id as any)}
                          style={{ padding: "13px 16px", borderRadius: 12, border: `1px solid ${aiPersonality === opt.id ? "var(--bm-accent-bd)" : "var(--bm-border)"}`, background: aiPersonality === opt.id ? "var(--bm-accent-dim)" : "var(--bm-bg3)", cursor: "pointer", fontFamily: "inherit", textAlign: "left", transition: "all 0.15s" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <div>
                              <div style={{ fontSize: 13, fontWeight: aiPersonality === opt.id ? 700 : 500, color: aiPersonality === opt.id ? "var(--bm-accent)" : "var(--bm-text2)", marginBottom: 3 }}>{opt.label}</div>
                              <div style={{ fontSize: 11, color: "var(--bm-text3)" }}>{opt.desc}</div>
                            </div>
