...
function DashboardPage() {
  ...
  // Streak: read from localStorage immediately so it renders on first paint,
  // then updates when overview data loads from Supabase
  ...
  useEffect(() => {
    if (overview?.founderStreakDays !== undefined) {
      setStreak(overview.founderStreakDays);
      localStorage.setItem("bm_streak", String(overview.founderStreakDays));
    }
  ...
  if (pL || sL || oL) return <BuildMindLoader variant="card" label="Loading dashboard…" />;
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }}
      ...
      {/* AI Visual Insight — generates live dashboard analysis */}
      {activeProjectSummary && (
        <AIVisualWidget
          page="dashboard"
          intent="Show a visual startup momentum dashboard: score gauge, stage progress bar, streak visualization, and top 2 recommended actions for this week"
          context={{
            stage: activeStage,
            stageIndex,
            streak,
          ...
          data={{
            score: activeProjectSummary ? Math.min(100, Math.round((activeProjectSummary.progress ?? 0) + (activeProjectSummary.validation_strengths?.length ?? 0) * 8)) : 0,
            progress: activeProjectSummary?.progress ?? 0,
            tasksCompleted: activeProjectSummary?.tasksCompleted ?? 0,
            tasksTotal: activeProjectSummary?.tasksTotal ?? 0,
            project: activeProjectSummary?.title ?? "",
            strengths: activeProjectSummary?.validation_strengths ?? [],
          }}
          label="Generate dashboard insight"
          className="mb-1"
        />
      )}
      ...
      {/* Community activity teaser — social proof + retention */}
      <motion.div
        initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
        className="mt-2"
      >
        <div style={{background:"var(--bm-bg2)",border:"1px solid var(--bm-border)",borderRadius:10,padding:"12px 14px"}}>
          <div style={{fontSize:10,color:"var(--bm-text4)",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:8,fontWeight:600}}>🌍 Founder activity — right now</div>
          {[
            {flag:"🇬🇭",action:"Interviewed 3 users about payment friction",stage:"Validation",ago:"4m ago"},
            {flag:"🇳🇬",action:"Shipped MVP landing page — got first signup",stage:"MVP",ago:"12m ago"},
            {flag:"🇬🇧",action:"Ran Break My Startup — pivoted pricing model",stage:"Launch",ago:"31m ago"},
          ].map((item,i)=>(
            <div key={i} style={{display:"flex",gap:8,alignItems:"flex-start",padding:"6px 0",borderBottom:i<2?"1px solid var(--bm-border)":"none"}}>
              <span style={{fontSize:14,flexShrink:0,marginTop:1}}>{item.flag}</span>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:11,color:"var(--bm-text2)",lineHeight:1.4,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{item.action}</div>
                <div style={{fontSize:9,color:"var(--bm-text4)",marginTop:1}}>{item.stage} · {item.ago}</div>
              </div>
            </div>
          ))}
          <div style={{marginTop:8,fontSize:10,color:"var(--bm-text4)",textAlign:"center"}}>
            <a href="/explore" style={{color:"var(--bm-purple)",textDecoration:"none"}}>See the full founder feed →</a>
          </div>
        </div>
      </motion.div>
